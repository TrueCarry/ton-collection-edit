import { Blockchain } from '@ton-community/sandbox'
import { Address, Cell, internal, MessageRelaxed, SendMode } from 'ton-core'
import '@ton-community/test-utils' // register matchers
import { buildNftCollectionStateInit, Queries } from './NftCollection.data'

export const NftItemEditableCode =
  ''

export const NftItemCode =
  ''

describe('Test Batch mint', () => {
  test('Should mint 400 editable nfts', async () => {
    const blockchain = await Blockchain.create()

    const owner = await blockchain.treasury('owner')

    const collectionInit = buildNftCollectionStateInit({
      ownerAddress: owner.address,
      collectionContent: '/content',
      commonContent: '/base',
      nextItemIndex: 0n,
      nftItemCode: Cell.fromBoc(Buffer.from(NftItemEditableCode, 'hex'))[0], // Cell12.fromBoc(Buffer.from( NftItem.codeHex, 'hex'))[0],
      royaltyParams: {
        royaltyAddress: owner.address,
        royaltyBase: 1000,
        royaltyFactor: 50,
      },
    })

    await owner.send({
      value: 10n ** 9n,
      to: Address.parse(collectionInit.address.toString()),
      init: {
        code: collectionInit.stateInitMessage.code,
        data: collectionInit.stateInitMessage.data,
      },
    })

    const messageValue = (10000000n + 17500000n) * 100n
    const messages: MessageRelaxed[] = []
    for (let i = 0; i < 4; i++) {
      const mintBody = Queries.batchMintEditable({
        items: [...Array(100)].map((_, j) => {
          return {
            passAmount: 10000000n,
            content: `${j + i * 100}.json`,
            index: j + i * 100,
            ownerAddress: owner.address,
            editorAddress: owner.address,
          }
        }),
      })

      messages.push(
        internal({
          value: messageValue,
          to: Address.parse(collectionInit.address.toString()),
          body: mintBody,
          bounce: true,
        })
      )
    }

    await owner.sendMessages(messages, SendMode.PAY_GAS_SEPARATLY)
    const balance = await blockchain.getContract(Address.parse(collectionInit.address.toString()))
    const data = await balance.get('get_collection_data')

    expect((data.stack[0] as any).value).toEqual(400n)
  })

  test('Should mint 400 NON-editable nfts', async () => {
    const blockchain = await Blockchain.create()

    const owner = await blockchain.treasury('owner')

    const collectionInit = buildNftCollectionStateInit({
      ownerAddress: owner.address,
      collectionContent: '/content',
      commonContent: '/base',
      nextItemIndex: 0n,
      nftItemCode: Cell.fromBoc(Buffer.from(NftItemCode, 'hex'))[0], // Cell12.fromBoc(Buffer.from( NftItem.codeHex, 'hex'))[0],
      royaltyParams: {
        royaltyAddress: owner.address,
        royaltyBase: 1000,
        royaltyFactor: 50,
      },
    })

    await owner.send({
      value: 10n ** 9n,
      to: Address.parse(collectionInit.address.toString()),
      init: {
        code: collectionInit.stateInitMessage.code,
        data: collectionInit.stateInitMessage.data,
      },
    })

    const messageValue = (10000000n + 17500000n) * 100n
    const messages: MessageRelaxed[] = []
    for (let i = 0; i < 4; i++) {
      const mintBody = Queries.batchMint({
        items: [...Array(100)].map((_, j) => {
          return {
            passAmount: 10000000n,
            content: `${j + i * 100}.json`,
            index: j + i * 100,
            ownerAddress: owner.address,
            editorAddress: owner.address,
          }
        }),
      })

      messages.push(
        internal({
          value: messageValue,
          to: Address.parse(collectionInit.address.toString()),
          body: mintBody,
          bounce: true,
        })
      )
    }

    await owner.sendMessages(messages, SendMode.PAY_GAS_SEPARATLY)
    const balance = await blockchain.getContract(Address.parse(collectionInit.address.toString()))
    const data = await balance.get('get_collection_data')

    expect((data.stack[0] as any).value).toEqual(400n)
  })
})
